...
protected:
	...
	template<typename ReturnType=std::nullopt_t>
...
private:
	...
	static bool CopyComponentByName(std::string stringToHash, entt::entity from,entt::entity to);
	...
	static T DuplicateObject(T other) {
		...
		for (auto [id, storage] : Registry::Get().storage()) {
			...
			if (storage.contains(other.ID()) && !storage.contains(obj.ID())) {
				AddComponentByName(obj.ID(), GetComponentNameByID(id));
				CopyComponentByName( GetComponentNameByID(id), other.ID(), obj.ID());
			}
